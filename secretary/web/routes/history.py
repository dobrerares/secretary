...
from secretary.core.actions import undo_action, undo_batch
...
async def history_list(request: Request, session: AsyncSession = Depends(get_session)):
    ...
    result = await session.execute(select(ActionLog).order_by(ActionLog.created_at.desc()).limit(50))
...
async def history_undo_batch(request: Request, batch_id: str, session: AsyncSession = Depends(get_session)):
    await undo_batch(session, batch_id)
    await session.commit()
